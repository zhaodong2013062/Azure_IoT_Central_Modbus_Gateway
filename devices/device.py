...
class Device(object):
    ...
    def __init__(self, scope_id, app_key, device_id, device_name, logger, model_id=''):
        ...
        if model_id == '':
            dps.getConnectionString(self.device_id, self.app_key, self.scope_id, True, self.dps_registration_callback)
        else:
            dps.getConnectionString(self.device_id, self.app_key, config.SCOPE_ID, True, 
                self.dps_registration_callback, config.MODEL_ID)
        ...
        self.client.message_callback_add('$iothub/twin/res/#', self.get_twin_callback)

        # desired properties subscribe
        self.client.subscribe('$iothub/twin/PATCH/properties/desired/#')
        self.client.message_callback_add('$iothub/twin/PATCH/properties/desired/#', self.desired_twin_callback)

        # Direct method subscribe
        self.client.subscribe('$iothub/methods/POST/#')
        self.client.message_callback_add('$iothub/methods/POST/#', self.direct_method_callback)

        # Cloud to Device message subscribe
        self.client.subscribe('devices/{}/messages/devicebound/#'.format(config.DEVICE_NAME))
        self.client.message_callback_add('devices/{}/messages/devicebound/#'.format(config.DEVICE_NAME), self.c2d_callback)
    ...
    def dps_registration_callback(self, error, hub):
        if error != None:
    ...
    # Handlers
    def get_twin_callback(self, client, userdata, msg):
    ...
    def desired_twin_callback(self, client, userdata, msg):
        ...
        self.desired_ack(json_data, 200, 'completed')

    def c2d_callback(self, client, userdata, msg):
        """ Handler for Azure IoT Hub Cloud to Device message - not available from IoT Central
        """
        self.logger.info('Cloud to Device message')

    def direct_method_callback(self, client, userdata, msg):
        ...
        self.logger.info('Direct method - method: %s, parameters: %s'.format(method_name, parameters))
    ...
    def gen_sas_token(self, hub_host, device_name, key, token_timeout):
    ...
    def connect(self):
        ...
        password = self.gen_sas_token(self.iot_hub_hostname, self.device_id, device_key, config.SAS_TOKEN_TTL)
